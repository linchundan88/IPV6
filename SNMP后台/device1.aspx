<%@ Page Language="C#" AutoEventWireup="true" CodeFile="device1.aspx.cs" Inherits="iftable_tree" %>
...
<body>
    ...
    <div>
        ...
        <asp:Label ID="Label1" runat="server" Font-Size="Large" ForeColor="#000099" Text="本页面是为了演示的目的，Community String等数据不是真实的，功能也和真实情况完全一致。"></asp:Label>
        <br />
        <br />
...
<GroupingSettings CollapseAllTooltip="Collapse all groups"></GroupingSettings>
        ...
        <asp:SqlDataSource ID="SqlDataSource1" runat="server" ConnectionString="<%$ ConnectionStrings:network_managementConnectionString %>" DeleteCommand="DELETE FROM device WHERE id= ?" InsertCommand="INSERT INTO device1 ( device_ip ,  location, enabled, community_read ,  community_write ,  device_type ,    snmp_port ,  get_arp ,  get_mac_address ) VALUES (?, ?, ?, ?, ?, ?, ?, ?)" ProviderName="<%$ ConnectionStrings:network_managementConnectionString.ProviderName %>" SelectCommand="SELECT * FROM device1" UpdateCommand="UPDATE device1 SET  community_read  = ?,  community_write  = ?,  device_type  = ?,   snmp_port  = ?,  get_arp  = ?,  get_mac_address  = ?,device_ip  = ?,location  = ?, enabled  = ? WHERE  id=?">
            <DeleteParameters>
